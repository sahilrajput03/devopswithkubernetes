...
let isMyHostTesting = process.env.ss === 'sahil rajput'
let myHost = 'mongodb://localhost:27017/db-project1'
log('db uri ~sahil: ', process.env.DB_URI)
log('$ss, isMyHostTesting', isMyHostTesting)
...
app.get('/', (req, res) => {
	...
	const response = `
	  ...
	  <script>
		...
		let SERVER_URL = '${isMyHostTesting ? 'http://localhost:3001/api/todos' : '/api/todos'}'
		...
		async function myFun(){
			...
			const {data} = await axios.post(SERVER_URL, {todo})
			console.log({data})
